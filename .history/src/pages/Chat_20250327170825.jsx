...
import { FaHome, FaUserFriends, FaComments, FaCalendarAlt, FaSignOutAlt, FaSmile } from "react-icons/fa";
              ...
              <div className="flex-1 overflow-y-auto bg-white p-4 rounded-lg shadow-md w-3/4 mx-auto">
                {messages.map((msg) => (
                  <div key={msg.id} className={`mb-4 flex items-center space-x-2 ${msg.sender === user?.displayName ? 'justify-end' : ''}`}>
                    {msg.sender !== user?.displayName && <FaSmile className="text-yellow-500 text-2xl" />}
                    <div>
                      <p className="text-gray-700 font-semibold">{msg.sender} <span className="text-xs text-gray-400">{msg.timestamp?.seconds ? new Date(msg.timestamp.seconds * 1000).toLocaleTimeString() : "Just now"}</span></p>
